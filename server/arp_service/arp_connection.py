        ...
        try: 
            with Device(host=device_ip, user=DataBaseProps.TEST_USER, password=DataBaseProps.TEST_PASSWORD) as dev:
                ...
                for item in in_table:
                    result_array.append( [item.name, item.admin, item.oper, item.description] )
                df = pd.DataFrame(result_array)
                df.columns = ['interface', 'status', 'link', 'description']
                dev.close()
                return df
        ...
        try: 
            with Device(host=device_ip, user=DataBaseProps.TEST_USER, password=DataBaseProps.TEST_PASSWORD) as dev:
                ...
                for item in ne_table:
                    ...
                    if(item.local_parent != "-"):
                        result_array.append( [item.local_parent, item.remote_chassis_id, item.remote_sysname] )
                df = pd.DataFrame(result_array)
    ...
    def get_interfaces_table(device_ip):
        ...
        try:    
            ...
            for line in in_table.split("\n")[1:]:
                serie = line.split()
                # if(serie[0] != "Interface"):
                inter = serie[0]
                if(serie[1] == "admin"):
                    status = serie[1] + ' ' + serie[2]
                    link = serie[3]
                    description = ' '.join(serie[4:])
                else:
                    status = serie[1]
                    link = serie[2]
                    description = ' '.join(serie[3:])
                    
                result_array.append( [inter, status, link, description])
...
class DellARP():
    ...
    def get_arp_table(device_ip):
        try:
            ...
            for line in arp_table.split("\n")[3:]:
                serie = line.split()
                if (validate_mac(serie[3])):
                    new_serie = [ serie[3], serie[1], serie[(len(serie)-3)] + serie[(len(serie)-2)] ]
                    result_array.append(new_serie)
            df = pd.DataFrame(result_array)
            df.columns = ['mac_address',"ip_address","interface_name"]
            return df
    ...
    def get_ethernet_switching_table(device_ip):
        try:
            ...
            for line in es_table.split("\n")[2:]:
                serie = line.split()
                if (validate_mac(serie[1])):
                    new_serie = [ serie[1], serie[0], serie[(len(serie)-3)] + serie[(len(serie)-2)] ]
                    result_array.append(new_serie)
            df = pd.DataFrame(result_array)
            df.columns = ['mac_address', "vlan_name", "logical_interface"]
            return df
    ...
    def get_interfaces_table(device_ip):
        try:
            ...
            connect = ConnectHandler(**device)
            connect.enable()
            int_table = connect.send_command('show interfaces description')
            connect.disconnect()
            for line in int_table.split("\n")[3:]:
                serie = line.split()
                new_serie = [ (serie[0]+serie[1]) , serie[len(serie)-1]]
                result_array.append(new_serie)
            df = pd.DataFrame(result_array)
    ...
    def get_neighbors_table(device_ip):
        try:
            ...
            connect = ConnectHandler(**device)
            connect.enable()
            ne_table = connect.send_command('show lldp neighbors')
            connect.disconnect()
            for line in ne_table.split("\n")[3:]:
                serie = line.split()
                new_serie = [ serie[0]+"-"+serie[1], serie[3], serie[2] ]
                result_array.append(new_serie)
            df = pd.DataFrame(result_array)
            df.columns = ['local_interface', 'device_id', 'port_info']
            return df
...
class AristaARP():
    ''' ARISTA CONNECTION UTILITY '''

    def get_arp_table(device_ip):
        try:
            device = ConnectHandler(
                host=device_ip, username=DataBaseProps.TEST_USER,password=DataBaseProps.TEST_PASSWORD,device_type="arista_eos",
            )
            result_array = []
            arp_table = device.send_command('show arp')
            for line in arp_table.split("\n")[1:len(arp_table.split("\n"))]:
                serie = line.split()
                if (validate_mac(format_mac_address(serie[2]))):
                    new_serie = [ format_mac_address(serie[2]), serie[0], serie[3][:-1] ]
                    result_array.append(new_serie)
            df = pd.DataFrame(result_array)
            df.columns = ['mac_address',"ip_address","interface_name"]
            return df
    ...
    def get_ethernet_switching_table(device_ip):
        try:
            ...
            result_array = []
            es_table = device.send_command('show mac address-table')
            for line in es_table.split("\n")[5:-8]:
                serie = line.split()
                if (validate_mac(format_mac_address(serie[1]))):
                    new_serie = [ format_mac_address(serie[1]), serie[0], serie[3] ]
    ...
    def get_interfaces_table(device_ip):
        try:
            ...
            result_array = []
            int_table = device.send_command('show interfaces description')
            for line in int_table.split("\n")[1:]:
                serie = line.split()
                new_serie = []
                if(len(serie) == 4):
                    new_serie = [serie[0], serie [3]]
                elif (len(serie) == 5):
                    if(serie[3] == "notpresent"):
                        new_serie = [ serie[0], serie[4] ]
                    else:
                        new_serie = [ serie[0], serie[3]+" "+serie[4]  ]
                elif(len(serie) == 6):
                    new_serie = [ serie[0], serie[3]+" "+serie[4]+" "+serie[5] ]
                if(new_serie):
                    result_array.append(new_serie)
    ...
    def get_neighbors_table(device_ip):
        try:
            ...
            result_array = []
            ne_table = device.send_command('show lldp neighbors')
            for line in ne_table.split("\n")[7:]:
                serie = line.split()
                port_info = ""
                for x in range(1, len(serie)-2):
                    port_info += serie[x]+" "
                result_array.append( [serie[0], serie[-2], port_info] )
            df = pd.DataFrame(result_array)
            df.columns = ['local_interface', 'device_id', 'port_info']
            return df
