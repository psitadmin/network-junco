# SQLAlchemy imports
from sqlalchemy import Column, Integer, String, ForeignKey, PrimaryKeyConstraint, text, Enum, create_engine, literal, not_, func
...
def switch_loop_search(mac_address):
    ...
    with Session() as session:
        ...
        if(len(devices) == 1):
            # print("Sólo un resultado => ", devices)
            inter = session.query(Devices)\
                .filter(InterfacesTable.ip_source == devices[0].Devices.ip)\
                .filter(InterfacesTable.interface == devices[0].EthernetSwitchingTable.logical_interface)\
                .filter(InterfacesTable.description.contains(Devices.name))\
                .first()
            if(not inter):
            ...
            for dev in devices:
                ...
                if ( check_interface( dev.EthernetSwitchingTable.logical_interface ) ):
                    ...
                    if(neighbors is not None):
                        # print("\n\n Con vecinos = ", neighbors)
                        neighbor_result = neighbors.NeighborsTable.port_info
                        device_result = session.query(Devices).filter(literal(neighbor_result).contains(Devices.name)).all()
                        ...
                        # print("Sin vecinos")
                        inter = session.query(Devices)\
                            .filter(InterfacesTable.ip_source == dev.Devices.ip)\
                            .filter(InterfacesTable.interface == dev.EthernetSwitchingTable.logical_interface)\
                            .filter(InterfacesTable.description.contains(Devices.name))\
                            .all()
                        if(not inter):
                            result.append(dev)
...
def build_json_calambuco(device_found, devices_result):
    ...
    for i in range( len(devices_result) ):
        ...
        inter_dev = None
        if(inter):
            inter_dev = session.query(Devices)\
                .filter(InterfacesTable.ip_source == devices_result[i].Devices.ip)\
                .filter(InterfacesTable.interface == devices_result[i].EthernetSwitchingTable.logical_interface)\
                .filter(InterfacesTable.description.contains(Devices.name)).all()
