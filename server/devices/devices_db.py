...
def get_device_by_ip(device_ip):
    ...
    with Session() as session:
        result = session.query(Devices).filter(Devices.ip == device_ip).first()
        session.close()
        return result

def get_device_by_id(device_id):
    ''' Returns device that matchs id param '''
    with Session() as session:
        result =  session.query(Devices).filter(Devices.id == device_id).first()
        session.close()
        return result
...
def get_device_id_json(dev):
    # Creating JSON item
    data = {
        'id' : dev.id,
        'ip' : dev.ip,
        'name' : dev.name,
        'vendor' : VendorEnum(dev.vendor).name,
        'serial_number' : dev.serial_number,
        'device_model' : dev.device_model,
        'protocol' : DeviceConnectionEnum(dev.connection_type).name,
        'location' : dev.location,
        'device_type' : DeviceTypeEnum(dev.device_type).name,
        'date' : dev.date.strftime('%Y-%m-%d %H:%M:%S'),
    }
    return data
